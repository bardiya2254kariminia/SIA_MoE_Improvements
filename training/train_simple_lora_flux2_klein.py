...
from visual_analogy.utils.selective_lora import inject_base_lora_modules
...
def parse_args(print_args=False):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use Mixture-of-Experts LoRA on image-stream MLP layers instead of PEFT LoRA.",
    )
...
def main():
    ...
    if use_moe:
        ...
        # Inject standard BaseLoRALinear on non-MoE layers
        ...
        # Inject MoE LoRA on MoE target layers
            ...
            if accelerator.is_main_process:
                for model in models:
                    ...
                    if isinstance(unwrapped, Flux2Transformer2DModel):
                        # Save base LoRA (non-MoE layers)
                        from visual_analogy.utils.selective_lora import save_base_lora_state_dict
        ...
        # Collect params: base LoRA + MoE gate/experts
        from visual_analogy.utils.selective_lora import collect_base_lora_params
        ...
        print(f"[Optimizer] base LoRA params: {len(base_lora_params)}, "
              f"MoE params: {len(moe_params)}")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        for _, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                if use_moe:
                    # MoE path: custom LoRA injection, no PEFT adapter
